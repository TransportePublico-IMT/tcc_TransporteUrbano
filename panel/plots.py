...
plot_localizacao_sptrans.layout = html.Div(
    [
        ...
        dcc.Interval(id="loc-sptrans-update", interval=20000, n_intervals=0),
    ]
)
...
def update_onibus_historico(self):
    ...
    plot_info = {"Dia": data["dias_list"], "Quantidade": data["quantidade_list"]}
    ...
    ret = {
        "data": [data],
        "layout": go.Layout(
            margin=dict(t=10, b=30, l=30, r=0),
            height=220,
            hovermode="closest",
            uirevision=True,
        ),
...
def sp_trans_localizacao(data):
    ...
    plot_info_paradas = apis.paradas("http://localhost:8000", "/api/paradas/")
    ...
    velocidades = apis.sp_trans_velocidade(
        "http://localhost:8000", "/api/onibus-velocidade/ultimos/"
    )
    for velocidade in velocidades:

        if velocidade["vel_trecho"] is not None and velocidade["vel_via"] is not None:
