...
class TransitFeeds(object):
    ...
    def feeds(self, location=None, descendants=None, **kwargs):
        ...
        results = self._request_results(
            "getFeeds", location=location, descendants=descendants, **kwargs
        )
    ...
    def feed_versions(self, feed, err=None, warn=None, **kwargs):
        ...
        err = err or 1
        results = self._request_results(
            "getFeedVersions", feed=feed, warn=warn, err=err, **kwargs
        )
    ...
    def _request(self, operation, rargs=None, **params):
        ...
        rargs = rargs or {}
        r = self.get(
            self.base + operation, headers=self.headers, params=params, **rargs
        )
        return r
    ...
    def _request_results(self, operation, **params):
        ...
        if result["status"] != "OK":
            raise RuntimeError(
                "Transitfeeds: Attempted to fetch {}, got back status: {} ({})".format(
