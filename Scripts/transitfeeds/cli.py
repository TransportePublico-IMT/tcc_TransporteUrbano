...
def location_cli(api, **kwargs):
    ...
    if kwargs["list"]:
        ...
        rows.extend(
            [
                (loc.id, utf8(loc.title), utf8(loc.name), loc.coords[0], loc.coords[1])
...
def feed_cli(api, **kwargs):
    ...
    if kwargs.get("header"):
        row = (
            ("url",)
            if kwargs.get("bare")
            else ("feed-id", "published", "start-date", "end-date", "url")
        )
        rows.append(row)
...
def add_boilerplate(parser, kind, noun):
    parser.add_argument(
        "--key",
        default=os.environ.get("TRANSITFEEDS_API_KEY"),
        help=(
            "Transitfeeds API key. Can be passed as an argument or "
            "read from the TRANSITFEEDS_API_KEY environment variable"
        ),
    )
    parser.add_argument(
        "ids", type=str, nargs="*", default=None, help="One or more {}-ids".format(kind)
    )
    parser.add_argument(
        "-H", "--header", action="store_true", help="Add a header to the output"
    )
    parser.add_argument(
        "--bare",
        action="store_true",
        help="Return only the {0} {1}, omitting {0} metadata".format(kind, noun),
    )
...
def parse_args(input_args):
    ...
    location = subparsers.add_parser(
        "location", help=LOCATION_HELP, description=LOCATION_HELP
    )
    add_boilerplate(location, "location", "id")
    location.add_argument(
        "--list", action="store_true", help="Return a list of all locations"
    )
    location.set_defaults(func=location_cli)

    feed = subparsers.add_parser(
        "feed",
        description=FEED_HELP,
        help=FEED_HELP,
        epilog=(
    ...
    feed.add_argument(
        "--latest", action="store_true", help="Return only the URL of the newest feed"
    )
